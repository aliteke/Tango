...
class Tangle(object):
    def __init__(self, isgenesis=True, *args):
        ...
        if isgenesis:
            ...
            genesis_tx = Transaction('genesis_address', 'genesis_address', 21000000)
            genesis_tx_key = self.attach_transaction(genesis_tx, ['genesis_tx'])

            founding_txA = Transaction('genesis_address', 'founderA_address', 7000000)
            self.attach_transaction(founding_txA, [genesis_tx_key])

            founding_txB = Transaction('genesis_address', 'founderB_address', 7000000)
    ...
    def attach_transaction(self, transaction, confirmed_transactions):
        if self.is_valid_transaction(transaction):
            ...
            print str({'msg': 'Transaction successful. Will be revealed in 30 seconds'})
